# biogen_gui.py
# GUI ONLY: calls functions from main.py

import tkinter as tk
from tkinter import ttk, filedialog, messagebox

import main  # algorithms file (Part 1/2/3/3D + Batch FASTA)
...
class BioGenGUI(tk.Tk):
    ...
    def build_single_tab(self):
        ...
        ttk.Button(btns, text="Load FASTA (first record)", command=self.load_single_fasta).pack(side="left")
        ttk.Button(btns, text="Analyze", command=self.run_single_analysis).pack(side="left", padx=8)

        # NEW BUTTON: Batch FASTA
        ttk.Button(btns, text="Batch FASTA (ALL records)", command=self.run_batch_fasta).pack(side="left", padx=8)
    ...
    def load_single_fasta(self):
        """Loads only the FIRST record from a FASTA file into the input box."""
    ...
    # NEW: Batch FASTA analysis (ALL sequences)
    def run_batch_fasta(self):
        path = filedialog.askopenfilename(
        ...
        try:
            reports = main.analyze_fasta_records(path)
            summary = main.summarize_fasta_reports(reports)

            out = []
            out.append("=== FASTA FILE SUMMARY ===")
            out.append(str(summary))
            out.append("")

            out.append("=== PER-SEQUENCE REPORT ===")
            for r in reports:
                stats = r["stats"]
                out.append(f"> {r['id']}")
                out.append(f"Length: {int(stats['length'])} | GC%: {stats['GC%']:.2f} | AT%: {stats['AT%']:.2f}")
                out.append(f"Counts: A={int(stats['A'])} T={int(stats['T'])} C={int(stats['C'])} G={int(stats['G'])}")
                out.append(f"ORFs: {r['orf_count']} | Proteins: {len(r['proteins'])}")

                # show only first 3 proteins to keep UI readable
                if r["proteins"]:
                    show = r["proteins"][:3]
                    out.append("Proteins (first 3):")
                    for p in show:
                        out.append(f"  - len={len(p)}: {p}")
                    if len(r["proteins"]) > 3:
                        out.append(f"  ... (+{len(r['proteins'])-3} more)")
                out.append("")
    ...
    def load_compare_fasta(self, target_text: tk.Text):
        ...
        try:
            records = main.read_fasta_records(path)
            seq_id, seq = records[0]  # first record only for compare input
